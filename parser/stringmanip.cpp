...
namespace KP{
	...
	int replace(const char *src, char *new_src, const char *tag, const char *tag_replacement){
		...
		int len_src = strlen(src);
		int len_tag = strlen(tag);
		int len_tag_replacement = strlen(tag_replacement);
		...
		*new_src = new_src[amountMemory];
		for (int i = 0; i < len_src; i++) {
			if (*(src + i) == tag[0]) {
				if (strncmp((src + i), tag, len_tag) == 0) {
					*(src + i) == tag_replacement[0];
				}
			}
		}
		strncpy(new_src, src, amountMemory);
		return SUCCESS;
	}
}
